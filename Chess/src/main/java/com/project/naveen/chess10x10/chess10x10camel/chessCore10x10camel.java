...
public class chessCore10x10camel {
    // TYPES AND CLASSES
    private enum pieceType {
        pawn, bishop, knight, rook, queen, king,snake,camel
    };
    ...
    public class piece {
        ...
        public void setPieceType(pieceType newType)
            ...
            if (type != pieceType.pawn)
                ...
                switch (newType) {
                    case snake:
                        availMoves = new snake();
                        break;
                    case camel:
                        availMoves = new camel();
                        break;
                    case bishop:
        ...
        public boolean isValidMove(cell moveTo) {
            ...
            //if ((colour == objectColour.white && tryState == gameState.whiteCheck) || (colour == objectColour.black && tryState == gameState.blackCheck))
              //  return false;
            return true;
        }
    ...
    private class pawn implements availableMoves {
        ...
        public ArrayList<cell> getAvailableMoves(piece piece) {
            ...
            // white moves 8 -> 0; black moves 1 -> 9
            if (piece.getPieceColour() == objectColour.white) {
                ...
                if (currY == 8 && board[currX][currY - 1].getPiece() == null
    ...
    private class bishop implements availableMoves {
        public ArrayList<cell> getAvailableMoves(piece piece) {
            ...
            // right, down
            while ((currX + i < 10 && currY + i < 10)
                    && (board[currX + i][currY + i].getPiece() == null
    ...
    private class camel implements availableMoves {
        public ArrayList<cell> getAvailableMoves(piece piece) {
            if (piece.getLocation() == deadCell)
                return null;
            ArrayList<cell> retList = new ArrayList<cell>();
            ...
            while ((currX + i < 10 && currY + i < 10)
                ...
                int j = 1,currXJ = currX + i,currYJ = currY + i;
                // left, down
                while ((currXJ - j > -1 && currYJ + j < 10)
                        && (board[currXJ - j][currYJ + j].getPiece() == null
                        || board[currXJ - j][currYJ + j].getPiece().getPieceColour() != piece.getPieceColour())) {
                    if (board[currXJ - j][currYJ + j].getPiece() != null
                            && board[currXJ - j][currYJ + j].getPiece().getPieceColour() != piece.getPieceColour()) {
                        retList.add(board[currXJ - j][currYJ + j]);
                        break;
                    }
                    retList.add(board[currXJ - j][currYJ + j]);
                    j++;
    ...
    private class snake implements availableMoves {
        public ArrayList<cell> getAvailableMoves(piece piece) {
            if (piece.getLocation() == deadCell)
                return null;
            ArrayList<cell> retList = new ArrayList<cell>();
            ...
            //up
            while ((currY - i > -1 )
                    && ((board[currX ][currY - i].getPiece() == null || board[currX
                    ][currY - i].getPiece().getPieceColour() != piece.getPieceColour())
                    /*
                    &&(board[currX + (i-1)][currY - (i-1)].getPiece() == null ||
                    board[currX + (i-1)][currY - (i-1)].getPiece().getPieceColour() != piece.getPieceColour())
                    &&(board[currX - (i-1)][currY - (i-1)].getPiece() == null ||
                    board[currX + (i-1)][currY - (i-1)].getPiece().getPieceColour() != piece.getPieceColour())
                    */
            )) {
                ...
                /*
                // right
                if ((currX + j < 10 && currY - j > -1)
                        && (board[currX + j][currY - j].getPiece() == null || board[currX
                        + j][currY - j].getPiece().getPieceColour() != piece.getPieceColour())) {
                    if (board[currX + j][currY - j].getPiece() != null
                            && board[currX + j][currY - j].getPiece().getPieceColour() != piece.getPieceColour()) {
                        //retList.add(board[currX + j][currY - j]);
                        break;
                    }
                    //retList.add(board[currX + j][currY - j]);
                    j++;
            ...
            //down
            while ((currY + i < 10)
                    && (board[currX][currY + i].getPiece() == null ||
                    board[currX][currY + i].getPiece().getPieceColour() != piece.getPieceColour())) {
                if (board[currX][currY + i].getPiece() != null
                        && board[currX][currY + i].getPiece().getPieceColour() != piece.getPieceColour()) {
                    retList.add(board[currX][currY + i]);
            ...
            i = 1;
            // right, up
            while ((currX + 1 < 10 && currY - i > -1)
                    && (board[currX + 1][currY - i].getPiece() == null
                    || board[currX + 1][currY - i].getPiece().getPieceColour() != piece.getPieceColour())) {
                if (board[currX + 1][currY - i].getPiece() != null
                        && board[currX + 1][currY - i].getPiece().getPieceColour() != piece.getPieceColour()) {
                    retList.add(board[currX + 1][currY - i]);
                    break;
                }
                retList.add(board[currX + 1][currY - i]);
                i=i+2;
            ...
            // up, right
            while ((currX + i < 10 && currY - 1 > -1)
                    && (board[currX + i][currY - 1].getPiece() == null
                    || board[currX + i][currY - 1].getPiece().getPieceColour() != piece.getPieceColour())) {
                if (board[currX + i][currY - 1].getPiece() != null
                        && board[currX + i][currY - 1].getPiece().getPieceColour() != piece.getPieceColour()) {
                    retList.add(board[currX + i][currY - 1]);
                    break;
                }
                retList.add(board[currX + i][currY - 1]);
                i=i+2;
    ...
    private class king implements availableMoves {
        public ArrayList<cell> getAvailableMoves(piece piece) {
            ...
            if (currX < 9 && (piece.isValidMove(board[currX + 1][currY])))
            ...
            if (currY < 9 && (piece.isValidMove(board[currX][currY + 1])))
                retList.add(board[currX][currY + 1]);
        ...
        */
        ...
        for (int i = 0; i < 10; i++) {
            ...
            piece whitePawn = new piece(objectColour.white, pieceType.pawn,
                    board[i][8], new pawn(), "wpawn");
            whitePieces[i] = whitePawn;
            board[i][8].setPiece(whitePawn);
        ...
        // rooks
        whitePieces[10] = new piece(objectColour.white, pieceType.rook,
                board[0][9], new rook(), "wrook");
        board[0][9].setPiece(whitePieces[10]);
        whitePieces[11] = new piece(objectColour.white, pieceType.rook,
                board[9][9], new rook(), "wrook");
        board[9][9].setPiece(whitePieces[11]);
        blackPieces[10] = new piece(objectColour.black, pieceType.rook,
                board[0][0], new rook(), "brook");
        board[0][0].setPiece(blackPieces[10]);
        blackPieces[11] = new piece(objectColour.black, pieceType.rook,
                board[9][0], new rook(), "brook");
        board[9][0].setPiece(blackPieces[11]);

        // knights
        whitePieces[12] = new piece(objectColour.white, pieceType.knight,
                board[1][9], new knight(), "wknight");
        board[1][9].setPiece(whitePieces[12]);
        whitePieces[13] = new piece(objectColour.white, pieceType.knight,
                board[8][9], new knight(), "wknight");
        board[8][9].setPiece(whitePieces[13]);
        blackPieces[12] = new piece(objectColour.black, pieceType.knight,
                board[1][0], new knight(), "bknight");
        board[1][0].setPiece(blackPieces[12]);
        blackPieces[13] = new piece(objectColour.black, pieceType.knight,
                board[8][0], new knight(), "bknight");
        board[8][0].setPiece(blackPieces[13]);

        // bishops
        whitePieces[14] = new piece(objectColour.white, pieceType.bishop,
                board[3][9], new bishop(), "wbishop");
        board[3][9].setPiece(whitePieces[14]);
        whitePieces[15] = new piece(objectColour.white, pieceType.bishop,
                board[6][9], new bishop(), "wbishop");
        board[6][9].setPiece(whitePieces[15]);
        blackPieces[14] = new piece(objectColour.black, pieceType.bishop,
                board[3][0], new bishop(), "bbishop");
        board[3][0].setPiece(blackPieces[14]);
        blackPieces[15] = new piece(objectColour.black, pieceType.bishop,
                board[6][0], new bishop(), "bbishop");
        board[6][0].setPiece(blackPieces[15]);

        // queens
        whitePieces[16] = new piece(objectColour.white, pieceType.queen,
                board[5][9], new queen(), "wqueen");
        board[5][9].setPiece(whitePieces[16]);
        blackPieces[16] = new piece(objectColour.black, pieceType.queen,
                board[5][0], new queen(), "bqueen");
        board[5][0].setPiece(blackPieces[16]);
        // kings
        whitePieces[17] = new piece(objectColour.white, pieceType.king,
                board[4][9], new king(), "wking");
        board[4][9].setPiece(whitePieces[17]);
        blackPieces[17] = new piece(objectColour.black, pieceType.king,
                board[4][0], new king(), "bking");
        board[4][0].setPiece(blackPieces[17]);


        // camel
        whitePieces[18] = new piece(objectColour.white, pieceType.camel,
                board[2][9], new camel(), "w_camel");
        board[2][9].setPiece(whitePieces[18]);

        whitePieces[19] = new piece(objectColour.white, pieceType.camel,
                board[7][9], new camel(), "w_camel");
        board[7][9].setPiece(whitePieces[19]);

        blackPieces[18] = new piece(objectColour.black, pieceType.camel,
                board[2][0], new camel(), "b_camel");
        board[2][0].setPiece(blackPieces[18]);
        blackPieces[19] = new piece(objectColour.black, pieceType.camel,
                board[7][0], new camel(), "b_camel");
        board[7][0].setPiece(blackPieces[19]);
